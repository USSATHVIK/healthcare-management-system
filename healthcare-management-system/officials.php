...
// Check if the user is logged in and not a resident
if (!isset($_SESSION['username']) || $_SESSION['role'] == 'resident') {
    header("Location: dashboard.php"); // Redirect residents to dashboard or another page
    exit();
}

// Continue fetching officials' details only for non-residents
$query = "SELECT o.name AS 'official',
          ...
          LEFT JOIN tbl_chairmanship c ON o.chairmanship = c.id
          LEFT JOIN tbl_position p ON o.position = p.id";
													...
													<tr>
														<td>
															<div class="avatar avatar-sm">
																<span class="avatar-title rounded-circle border border-white" style="background-color: #4682B4"><?= ucwords($row['official'][0]) ?></span>
															</div>
